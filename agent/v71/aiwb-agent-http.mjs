...
function attachWebSocketControlReader(socket) {
...
export function createAgentDirectServer({ config = loadAgentDirectConfig(), control = invokeControl } = {}) {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/v1/health") {
        ...
        return json(response, 200, {
          ...runtimeHealth,
          protocolVersion: 2,
          capabilities: ["tasks", "binary-upload-v1", "events-v1"],
          transport: "https",
      ...
      if (request.method === "POST" && url.pathname === "/v1/tasks") {
        ...
        if (
          result.code !== 0 &&
          /__AIWB_AGENT_ERROR_CODE__execution_context_required(?:\r?\n|$)/.test(controlDetail)
        ) {
          // The App will recreate the same task over SSH. Do not let this
          // headless, status-less preparation appear as an active task.
          removeUnstartedPreparedTask(taskId);
          return json(response, 409, {
            error: {
              code: "execution_context_required",
              message: "macOS 任务必须从 SSH 或交互式用户上下文启动。",
            },
          });
        }
        if (
          result.code !== 0 &&
          /__AIWB_AGENT_ERROR_CODE__capacity_reached(?:\r?\n|$)/.test(controlDetail)
        ) {
          if (!readText(join(taskPath(taskId), "status"))) removeUnstartedPreparedTask(taskId);
          return json(response, 429, {
            error: {
              code: "capacity_reached",
              message: "Agent 当前并发任务已达到上限，请稍后重试。",
            },
          });
        }
        ...
        if (result.code !== 0 || task.status === "missing") {
          return json(response, 502, { error: { code: "task_create_failed", message: controlDetail || "Agent 创建任务失败。" } });
        }
  ...
  server.on("upgrade", (request, socket) => {
    ...
    attachWebSocketControlReader(socket);
