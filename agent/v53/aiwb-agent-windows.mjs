...
function stopDaemon() {
  stopPidFile(SERVICE_PID_FILE);
  stopPidFile(PID_FILE);
  stopPidFile(HTTP_PID_FILE);
  stopPidFile(UPDATER_PID_FILE);
...
function pidFileStatus(file) {
  return isAlive(readTrim(file)) ? "running" : "stopped";
}
...
function runtimeReady() {
  return [SERVICE_PID_FILE, PID_FILE, HTTP_PID_FILE, UPDATER_PID_FILE].every((file) => pidFileStatus(file) === "running")
...
function emitRuntimeStatuses() {
  console.log("__AIWB_AGENT_SERVICE_PROCESS_STATUS__" + pidFileStatus(SERVICE_PID_FILE));
  console.log("__AIWB_AGENT_DAEMON_STATUS__" + daemonStatus());
  console.log("__AIWB_AGENT_DAEMON_HEARTBEAT__" + readTrim(HEARTBEAT_FILE));
  console.log("__AIWB_AGENT_HTTP_STATUS__" + pidFileStatus(HTTP_PID_FILE));
  console.log("__AIWB_AGENT_UPDATER_STATUS__" + pidFileStatus(UPDATER_PID_FILE));
}
...
function setStatus(id, status, exitCode = "") {
  ...
  if (["done", "error", "cancelled"].includes(status)) scheduleTerminalNotification(id);
}
...
async function runChild(executable, args, input = "", envOverrides = {}) {
...
async function runCliSpec(id, spec) {
  ...
  if (process.platform === "win32") {
    if (!tool.executable.toLowerCase().endsWith("powershell.exe")) {
      result = await runChild(tool.executable, args, "", cliEnv);
    } else {
    ...
    result = await runChild("powershell.exe", ["-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", powershellScript], "", cliEnv);
    }
  } else {
    result = await runChild(tool.executable, [...tool.prefix, ...args], "", cliEnv);
  }
  let output = "";
...
async function daemon() {
  ...
  while (true) {
    if (installedVersion() !== VERSION) {
      log("daemon version superseded pid=" + process.pid + " version=" + VERSION);
      const next = spawn(process.execPath, [process.argv[1], "service-run"], { detached: true, stdio: "ignore", windowsHide: true });
      next.unref();
      return;
    }
...
async function serviceRun() {
  ...
  const startManaged = (name, scriptPath) => {
    ...
    child.once("close", () => managed.delete(name));
    child.once("error", () => managed.delete(name));
  };
  ...
  try {
    ...
    if (readTrim(SERVICE_PID_FILE) === String(process.pid)) write(SERVICE_PID_FILE, "");
  }
}
