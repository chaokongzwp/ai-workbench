...
function updateConversation(id) {
  ...
  for (const name of ["name", "workdir", "agent_id", "model", "created_at", "queued_at", "started_at", "runner_started_at", "finished_at", "exit_code"]) {
...
function fingerprint(id) {
  const directory = taskDir(id);
  const content = [taskStatus(id), read(path.join(directory, "output.log")), read(path.join(directory, "bootstrap.log"))].join("\n");
...
async function runTask(id) {
  ...
  try {
    const spec = commandSpecFromTask(id);
    let result;
